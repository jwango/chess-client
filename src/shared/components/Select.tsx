...
export const Select = <T, O extends SelectOption<T>>({ options, placeholder = 'None', selectedOption, onSelect }: SelectProps<T, O>) => {
  const [open, setOpen] = useState(false);

  return <div className="w-72 relative">
  <Listbox value={selectedOption} onChange={onSelect} disabled={!options?.length}>
    <Listbox.Button
