...
export const Select = <T, O extends SelectOption<T>>({ key, options, placeholder = 'None', onSelect }: SelectProps<T, O>) => {
  ...
  <Listbox value={selectedOption} onChange={setSelectedOption} disabled={!options?.length}>
    ...
    <Listbox.Options className="absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-lg shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-sm z-50">
      {(options || []).map((option) => (
        <Listbox.Option
          key={option.id}
          value={option}
